...
class ContentLoss(nn.Module):
    def __init__(self, lammda_ori=0, max_bias=1, min_weight=0.1):
        super(ContentLoss, self).__init__()
        self.criterion = nn.MSELoss()
        self.max_bias = max_bias
        self.lammda_ori = lammda_ori
    ...
    def content(self, input, gan_out):
        ...
        for x_it, gan_out_it in zip(input, gan_out):
            ...
            if (abs(weight[:, 3]).mean() > self.max_bias or abs(
                    weight[:3, :3]).mean() < self.min_weight) and self.last_weight is not None:
    ...
    def forward(self, input, gan_out):
        loss = self.criterion(gan_out, self.content(input, gan_out).detach())
        if self.lammda_ori > 0:
            gan_out = F.instance_norm(gan_out)
            input = F.instance_norm(input)
            loss += self.lammda_ori * (1 - (gan_out * input).mean())
        return loss
